...
| <a name="input_karpenter_enable_inline_policy"></a> [karpenter\_enable\_inline\_policy](#input\_karpenter\_enable\_inline\_policy) | If true, render the Karpenter controller policy as an inline role policy (aws\_iam\_role\_policy) instead of a customer-managed policy (aws\_iam\_policy). Use this to work around the 6,144-character managed-policy quota; inline role policies have a combined 10,240-character per-role limit. Forwarded to the upstream terraform-aws-modules/eks//modules/karpenter `enable_inline_policy` variable (added in v21.8.0). | `bool` | `false` | no |
